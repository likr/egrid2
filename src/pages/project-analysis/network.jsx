...
class Network extends React.Component {
  render() {
    ...
    const content = (
      <g>
        <g>{edges.map(({u, v, points, d, ud, vd}) => {
          return <Edge key={`${u}:${v}`} points={points} strokeColor={edgeColor(ud, vd)} {...d}/>
        })}</g>
        <g>{vertices.map(({u, x, y, d}) => {
          return <Vertex key={u} x={x} y={y} strokeColor={vertexColor(d)} {...d} onClick={this.handleClickVertex.bind(this, u)}/>
        })}</g>
      </g>
    );
    return (
